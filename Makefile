...
LDFLAGS := -ldflags "-X github.com/Tragidra/logstruct/pkg/version.Version=$(VERSION) -X github.com/Tragidra/logstruct/pkg/version.Commit=$(COMMIT)"

help:
	@echo "Targets:"
	@echo "  build         - compile logstruct binary"
	@echo "  test          - run unit tests"
	@echo "  test-race     - run tests with race detector"
	@echo "  lint          - run golangci-lint"
	@echo "  dev-up        - docker compose up (postgres, for integration tests)"
	@echo "  dev-down      - docker compose down"
	@echo "  migrate       - apply migrations to ./logstruct.db (SQLite default or PG)"
...
build:
	$(GO) build $(LDFLAGS) -o bin/$(BINARY) ./cmd/logstruct
